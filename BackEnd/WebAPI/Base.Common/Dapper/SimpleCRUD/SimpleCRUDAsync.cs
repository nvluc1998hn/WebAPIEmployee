...
namespace Base.Common.Dapper.SimpleCRUD
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static async Task<T> GetSingleByConditionAsync<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var name = GetTableName(currenttype);
        ...
        public static Task<IEnumerable<T>> GetTopListAsync<T>(this IDbConnection connection, int number, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
        ...
        public static Task<IEnumerable<T>> GetListAsync<T>(this IDbConnection connection, string conditions, string orderby, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
        ...
        {
            return connection.InsertAsync<int?, TEntity>(entityToInsert, transaction, commandTimeout);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The ID (primary key) of the newly inserted record if it is identity using the defined type, otherwise null</returns>
        public static async Task<List<TKey>> InsertListAsync<TKey, TEntity>(this IDbConnection connection, List<TEntity> entitysToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (entitysToInsert.Any())
            {
                var entity = entitysToInsert[0];
                ...
                // Lấy type Key
                string typeKey = "";
                var idType = idProps[0].PropertyType;
                if (idType.Name.Contains("Int"))
                {
                    typeKey = "INT";
                }
                else if (idType.Name.Contains("Guid"))
                {
                ...
                sb.Append($"CREATE TABLE #RowsInserted(Id {typeKey}); ");
                sb.AppendFormat("insert into {0} ", name);
                sb.Append(" (");
                BuildInsertParameters<TEntity>(sb);
                sb.Append(") ");
                var keysTable = idProps[0].GetCustomAttributes(true);
                string key = "";
                foreach (var keyReal in keysTable)
                {
                    if (keyReal.GetType() == typeof(ColumnAttribute))
                    {
                        key = ((ColumnAttribute)keyReal).Name;
                        break;
                    }
                }
                sb.AppendFormat(" OUTPUT inserted.{0} into #RowsInserted ", key);
                sb.Append(" values");

                List<StringBuilder> values = new List<StringBuilder>();
                foreach (var entityToInsert in entitysToInsert)
                {
                    var value = new StringBuilder();
                    value.Append(" (");
                    value.Append(BuildInsertValuesByValue(entityToInsert));
                    value.Append(")");
                    values.Add(value);
                }
                ...
                if (idType == typeof(TKey))
                {
                    foreach (IDictionary<string, object> row in rows)
                    {
                        ...
                        {
                            var id = Convert.ChangeType(pair.Value, idType);
                            lstIds.Add((TKey)id);
                        }
                    }
                }
        ...
        public static async Task<int> InsertAsyncCustomKey<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            sb.AppendFormat("insert into {0}", name);
            sb.Append(" (");
            BuildInsertParametersCutomKey<TEntity>(sb);
            sb.Append(") ");
            sb.Append("values");
            sb.Append(" (");
            BuildInsertValuesCustomKey<TEntity>(sb);
            sb.Append(")");

            if (Debugger.IsAttached)
                Trace.WriteLine(string.Format("Insert: {0}", sb));

            return await connection.ExecuteAsync(sb.ToString(), entityToInsert, transaction, commandTimeout);
        }
        ...
        public static Task<int> UpdateAsync<TEntity>(this IDbConnection connection, List<TEntity> entitiesToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, System.Threading.CancellationToken? token = null)
        {
            ...
            foreach (var entity in entitiesToUpdate)
            {
                ...
                sb.AppendFormat(" set ");
                BuildUpdateSetWithValue(entity, sb);
                sb.Append(" where ");
                BuildWhereWithValue<TEntity>(sb, idProps, entity);
            }

            if (Debugger.IsAttached)
                Trace.WriteLine(string.Format("Update: {0}", sb));
            var a = sb.ToString();
            return connection.ExecuteAsync(sb.ToString(), transaction: transaction, commandTimeout: commandTimeout);
        }

        public static Task<int> UpdateListAsync<T>(this IDbConnection connection, string conditions, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            if (string.IsNullOrEmpty(conditions))
                throw new ArgumentException("UpdateList<T> requires a where clause");
            if (!conditions.ToLower().Contains("where"))
                throw new ArgumentException("UpdateList<T> requires a where clause and must contain the WHERE keyword");
        ...
        /// <summary>
        ///Cảnh báo: Không nên dùng. Chỉ dùng cho bảng có 1 dòng dữ liệu -> áp dụng cho KPI bảng [KPI.Setting4IdlingEvalItems].
        ///  <para>Updates a record or records in the database asynchronously</para>
        ///  <para>By default updates records in the table matching the class name</para>
        ///  <para>-Table name can be overridden by adding an attribute on your class [Table("YourTableName")]</para>
        ///  <para>Updates records where the Id property and properties with the [Key] attribute match those in the database.</para>
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The number of affected records</returns>
        public static Task<int> UpdateWithoutConditionAsync<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, System.Threading.CancellationToken? token = null)
        {
            var idProps = GetIdProperties(entityToUpdate).ToList();
        ...
        public static async Task<IEnumerable<T>> QueryDapperAsync<T>(this IDbConnection connection, string sql, object param = null)
        {
            return await connection.QueryAsync<T>(sql, param);
        }

        public static async Task<IEnumerable<T>> QueryDapperStoreProcAsync<T>(this IDbConnection connection, string store, object param = null)
        {
            return await connection.QueryAsync<T>(store, param, commandType: CommandType.StoredProcedure);
        }

        public static async Task<T> QueryFirstOrDefaultAsync<T>(this IDbConnection connection, string sql, object parameters = null)
        {
            return await connection.QueryFirstOrDefaultAsync<T>(sql, parameters);
        }

        public static async Task<T> QueryFirstOrDefaultStoreProcAsync<T>(this IDbConnection connection, string store, object parameters = null)
        {
            return await connection.QueryFirstOrDefaultAsync<T>(store, parameters, commandType: CommandType.StoredProcedure);
        }

        public static async Task<int> ExecuteDapperAsync(this IDbConnection connection, string sql, object param = null)
        {
            try
            {
                if (param == null)
        ...
        public static async Task<int> ExecuteDapperStoreProcAsync(this IDbConnection connection, string store, object param = null)
        {
            try
            {
                if (param == null)
                ...
                var result = await connection.ExecuteAsync(store, param, commandType: CommandType.StoredProcedure);
                return result;
            }
            catch
            {
        ...
        public static async Task<IEnumerable<T>> QueryDapperStoreProcReportAsync<T>(this IDbConnection connection, string store, object param = null)
        {
            return await connection.QueryAsync<T>(store, param, commandType: CommandType.StoredProcedure);
        }

        public static Task<int> ExecuteDapperStoreProcAsync<T>(this IDbConnection connection, string store, object param = null)
        {
            return connection.ExecuteAsync(store, param, commandType: CommandType.StoredProcedure);
        }

        /// <summary>
        /// Tương tự như hàm GetListAsync, nhưng với trường hợp câu điều kiện conditions có join với bảng khác
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="connection"></param>
        /// <param name="conditions"></param>
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static Task<IEnumerable<T>> GetListAsyncWithJoinConditions<T>(this IDbConnection connection, string conditions, string orderby, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
            ...
            sb.Append("Select ");
            //create a new empty instance of the type to get the base properties
            BuildSelectWithJoinCondition<T>(sb, GetScaffoldableProperties<T>().ToArray());
            sb.AppendFormat(" from {0}", name);
            if (string.IsNullOrEmpty(orderby))
            {
                orderby = GetColumnName(idProps.First());
            ...
            sb.Append(" " + conditions);
            sb.Append(" " + "ORDER BY " + name + "." + orderby);
            if (Debugger.IsAttached)
                Trace.WriteLine(string.Format("GetList<{0}>: {1}", currenttype, sb));
            var a = sb.ToString();
