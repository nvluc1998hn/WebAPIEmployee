...
namespace EmployeeManagement.Database.Dapper.SimpleCRUD
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static T GetSingleByCondition<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var name = GetTableName(currenttype);
        ...
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, string conditions, string orderby, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
        ...
        public static IEnumerable<T> QueryDapper<T>(this IDbConnection connection, string sql, object param = null)
        {
            return connection.Query<T>(sql, param);
        }

        public static T QueryFirstOrDefault<T>(this IDbConnection connection, string sql, object parameters = null)
        {
            return connection.QueryFirstOrDefault<T>(sql, parameters);
        }

        public static T QueryFirstOrDefaultStoreProc<T>(this IDbConnection connection, string store, object parameters = null)
        {
            return connection.QueryFirstOrDefault<T>(store, parameters, commandType: CommandType.StoredProcedure);
        }

        public static IEnumerable<T> QueryDapperStoreProc<T>(this IDbConnection connection, string store, object param = null)
        {
            return connection.Query<T>(store, param, commandType: CommandType.StoredProcedure);
        }

        public static int ExecuteDapper(this IDbConnection connection, string sql, object param = null)
        {
            try
            {
                if (param == null)
        ...
        public static int ExecuteDapperStoreProc(this IDbConnection connection, string store, object param = null)
        {
            try
            {
                if (param == null)
                ...
                var result = connection.Execute(store, param, commandType: CommandType.StoredProcedure);
                return result;
            }
            catch
            {
        ...
        public static IEnumerable<T> QueryDapperStoreProcReport<T>(this IDbConnection connection, string store, object param = null)
        {
            return connection.Query<T>(store, param, commandType: CommandType.StoredProcedure);
        }

        public static Tuple<IEnumerable<T1>, IEnumerable<T2>, IEnumerable<T3>, IEnumerable<T4>> QueryMultiple<T1, T2, T3, T4>(this IDbConnection connection, string store, object param = null)
        {
            Tuple<IEnumerable<T1>, IEnumerable<T2>, IEnumerable<T3>, IEnumerable<T4>> result = null;
            try
            {
                using (var qr = connection.QueryMultiple(store, param, commandType: CommandType.StoredProcedure))
                {
                    var t1 = qr.Read<T1>();
                    var t2 = qr.Read<T2>();
                    var t3 = qr.Read<T3>();
        ...
        private static void BuildInsertValuesCustomKey<T>(StringBuilder masterSb)
        {
        ...
        public static Tuple<IEnumerable<T1>, IEnumerable<T2>, IEnumerable<T3>> QueryMultiple<T1, T2, T3>(this IDbConnection connection, string store, object param = null)
        {
        ...
        private static void BuildUpdateSetWithValue<T>(T entityToUpdate, StringBuilder sb)
        {
            var nonIdProps = GetUpdateableProperties(entityToUpdate).ToArray();
            ...
            for (var i = 0; i < nonIdProps.Length; i++)
            {
                ...
                if (propValue == null)
                {
                    sb.AppendFormat("{0} = {1}", GetColumnName(property), "NULL");
                }
                else
                {
                    if (typeof(DateTime?).IsAssignableFrom(property.PropertyType))
                    {
                        sb.AppendFormat("{0} = N'{1}'", GetColumnName(property), ((DateTime)propValue).ToString("yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture));
                    }
                    else
                    {
                        sb.AppendFormat("{0} = N'{1}'", GetColumnName(property), propValue);
                    }
                }
                if (i < nonIdProps.Length - 1)
                    sb.AppendFormat(", ");
        ...
        private static void BuildInsertParametersCutomKey<T>(StringBuilder masterSb)
        {
        ...
        private static void BuildWhereWithValue<TEntity>(StringBuilder sb, IEnumerable<PropertyInfo> idProps, object entity = null)
        {
            var propertyInfos = idProps.ToArray();
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                for (var x = 0; x < sourceProperties.Count(); x++)
                {
                    if (sourceProperties.ElementAt(x).Name == propertyToUse.Name)
                    {
                        if (entity != null && propertyToUse.CanRead && (propertyToUse.GetValue(entity, null) == null || propertyToUse.GetValue(entity, null) == DBNull.Value))
                        {
                            useIsNull = true;
                        }
                        propertyToUse = sourceProperties.ElementAt(x);
        ...
        //Not marked with [IgnoreInsert]
        //Not marked with [NotMapped]
        private static StringBuilder BuildInsertValuesByValue<T>(T entity)
        {
            ...
            for (var i = 0; i < props.Count(); i++)
            {
                ...
                //if (property.Name.Equals("Id", StringComparison.OrdinalIgnoreCase) && property.GetCustomAttributes(true).All(attr => attr.GetType().Name != typeof(RequiredAttribute).Name) && property.PropertyType != typeof(Guid))
                //    continue;
                if (property.PropertyType.Name == "Guid" || property.PropertyType.Name == "Boolean")
                {
                    sb.AppendFormat("'{0}'", property.GetValue(entity).ToString());
                }
                else if (property.PropertyType.Name == "DateTime")
                {
                    var stringDateValue = ((DateTime)(property.GetValue(entity))).ToString("yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture);
                    sb.AppendFormat("'{0}'", stringDateValue);
                }
                // thainh2 - check các trường hợp null
                else if (property.PropertyType.Name == "String" || property.PropertyType.ToString().Contains("System.Nullable`1[System.DateTime]") || property.PropertyType.ToString().Contains("System.Nullable`1[System.Boolean]") || property.PropertyType.ToString().Contains("System.Nullable`1[System.Guid]"))
                {
                    ...
                    {
                        sb.AppendFormat("N'{0}'", property.GetValue(entity).ToString());
                    }
                }
                else
                {
                    sb.AppendFormat("{0}", property.GetValue(entity) == null ? "NULL" : property.GetValue(entity));
                }
                if (i < props.Count() - 1)
                    sb.Append(", ");
            }
        ...
        /// <summary>
        /// Tạo câu truy vấn select trong 1 bảng với trường hợp câu điều kiện join với bảng khác
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="masterSb"></param>
        /// <param name="props"></param>
        private static void BuildSelectWithJoinCondition<T>(StringBuilder masterSb, IEnumerable<PropertyInfo> props)
        {
